      ...
      <a-layout-content :style="{padding:'0 24px', height:'100%'}">
        ...
        >
          <span slot="audit" slot-scope="text, record">
            ...
            >{{ $t('common.auditOk') }}</a-tag>
            <a-tag
              v-if="text === 1"
              color="#108ee9"
              @click="showAuditRecord(record.id)"
            >{{ $t('common.auditWait') }}</a-tag>
            <a-tag
              v-if="text === 2"
              color="#f50"
              @click="showAuditRecord(record.id)"
            >{{ $t('common.auditFail') }}</a-tag>
          </span>
...
export default {
  ...
  methods: {
    handleEdit () {
      this.$router.push({ name: 'taskEdit', params: { id: this.selectedRowKeys[0] } })
    },
    onNewTask (screen) {
      this.$router.push({ name: 'taskEdit', params: { screen: screen, mgroupId: this.$refs.mgroup.getSelected() } })
    },
    handleAudit () {
