<template>
  ...
  >
    <div
      v-for="ha in actualHandles"
      :key="ha"
      :class="[classNameHandle, classNameHandle + '-' + ha]"
      :style="{display: enabled ? 'block' : 'none'}"
      @dblclick="onDbClick"
      @mousedown.stop.prevent="handleDown(ha, $event)"
      @touchstart.stop.prevent="handleTouchDown(ha, $event)"
    >
...
export default {
  ...
  methods: {
    ...
    deselect (e) {
      ...
      if (!this.$el.contains(target) && !regex.test(target.className) && (typeof target.className === 'string' ? target.className.indexOf('area-') >= 0 : false)) {
        if (this.enabled && !this.preventDeactivation) {
          this.enabled = false
          this.$emit('deactivated', this.area)
          this.$emit('update:active', false)
        }
        removeEvent(document.documentElement, eventsFor.move, this.handleMove)
      }
